...
export function useLightbox() {
  ...
  const isNextButtonDisabled =
    currentIndexRef.current >= imagesRef.current.length - 1;

  const updateTransform = useCallback(() => {
    setImageTransform(
      `translate(${translateXRef.current}px, ${translateYRef.current}px) scale(${scaleRef.current})`,
    );
  }, []);

  const openLightbox = useCallback(
    (src: string, allImages: string[], index: number) => {
