---
title: "本地图转网络地址发送"
description: "QQ 群不支持本地图发送时，将本地图转换为网络地址再发送的两种方法。"
icon: Link2
---
...
1. 进入[网页控制台](../started/web-console)，找到左侧【修改设定】，点击进入
...
打开该文件（或者使用[网页控制台](../started/web-console)）查看其配置，会发现默认状态下`qqgroup`的配置项中，默认发送方式为`link`，而其他平台均为`base64`。
