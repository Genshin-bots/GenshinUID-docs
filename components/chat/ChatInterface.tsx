...
export function ChatInterface() {
  ...
  const { isMarkdownMode, renderContent, toggleMarkdownMode } =
    useMessageRenderer();
  const {
    isGroupMode,
    groupId,
  ...
  const onWsMessage = useCallback(
    (data: any) => {
      if (data.content) {
        ...
        if (nodeContent && Array.isArray(nodeContent.data)) {
          const nodeData: NodeContent[] = nodeContent.data.map((msg: any) => ({
            ...
            avatar:
              msg.avatar ||
              msg.sender?.avatar ||
              'https://s2.loli.net/2023/03/25/bareSdYcsmRPOyZ.png',
            messages: Array.isArray(msg.messages)
              ? msg.messages
              : [{ type: msg.type || 'text', data: msg.data || '' }],
          }));
          setMessages((prev) => [
            ...prev,
            {
              ...
              sender: data.sender || {
                nickname: '服务器',
                avatar: 'https://s2.loli.net/2023/03/25/bareSdYcsmRPOyZ.png',
              },
            },
          ]);
          return;
        ...
        const mainContent = data.content.filter(
          (c: any) => c.type !== 'buttons',
        );
        const buttonContent = data.content.find(
          (c: any) => c.type === 'buttons',
        );
  ...
  const {
    ...
    cancelConnection,
  } = useWebSocket(onWsMessage, onWsError);
  ...
  const sendRichMessage = useCallback(() => {
    ...
    const messageToSend = {
      ...
      sender: {
        nickname: '我',
        avatar: 'https://s2.loli.net/2023/10/05/GHjJNWBP4nezgIU.png',
      },
      user_pm: 3,
      content,
    };
  ...
  const handleButtonClick = useCallback(
    (button: { text: string; data: string; style?: number }) => {
      if (connectionStatus !== 'connected') return;
      const messageToSend = {
        bot_id: 'web',
        bot_self_id: 'web-client-001',
    ...
    <div className="fd-chat-container">
      ...
      <ChatInputArea
        ...
        onTriggerFileUpload={(type) =>
          fileUpload.triggerFileUpload(fileUpload.fileInputRef, type)
        }
